...
Route::get('/download', function () {
    return Storage::download('Profile.pdf');
});
...
Route::get('/', function () {
    ...
    // dd($posts);
    return view('welcome', compact(['projects', 'firstBanner', 'secondBanner', 'posts', 'categories']));
})->name('HOME::PAGE');
...
Route::get('/single-project/{project_id}', function ($project_id) {
    ...
    if ($totalRecords > 0) {
        ...
        $secondSlot = $chunks[1];
        $lastedSlot = $chunks[2];
        return view('pages.single-project', compact(['project', 'firstSlot', 'secondSlot', 'lastedSlot']));
    }
    // dd($project);
    return view('pages.single-project', compact(['project', 'firstSlot', 'secondSlot', 'lastedSlot']));
...
Route::middleware('auth')->group(function () {
    ...
    // project images
    Route::get('all-projects-images', [ProjectImageController::class, 'index'])->name('ALL::PROJECT::IMAGES');
    Route::get('add-projects-images', [ProjectImageController::class, 'create'])->name('ADD::PROJECT::IMAGES');
    Route::post('store-projects-images', [ProjectImageController::class, 'store'])->name('STORE::PROJECT::IMAGES');
    Route::get('delete/news/{news_id}', [ProjectImageController::class, 'destroy'])->name('DELETE::PROJECT::IMAGES');
